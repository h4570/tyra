...
void PipelineDirLightsBag::setDirectionalLightColors(
    Color* colors, const unsigned char& count) {
  for (unsigned char i = 0; i < count; i++)
    setDirectionalLightColor(colors[i], i);
}

void PipelineDirLightsBag::setDirectionalLightDirections(
    Vec4* directions, const unsigned char& count) {
  for (unsigned char i = 0; i < count; i++)
    setDirectionalLightDirection(directions[i], i);
}

void PipelineDirLightsBag::setDirectionalLightColor(
    const Color& color, const unsigned char& index) {
...
void PipelineDirLightsBag::setDirectionalLightDirection(
    const Vec4& direction, const unsigned char& index) {
  TYRA_ASSERT(mode != Manual,
              "Directional lights cannot be set in manual mode");
  TYRA_ASSERT(index < 3, "There are max 3 directional lights");
